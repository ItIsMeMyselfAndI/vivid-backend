...
@router.get("")
def get_profile(user_id: str, auth: Auth):
    token, user_supabase = auth
    response = (
        user_supabase.table("profile").select("*").match({"id": user_id}).execute()
    )
    print(response)
    return response
...
@router.post("")
def create_profile(data: CreateProfile, auth: Auth):
    token, user_supabase = auth
...
@router.put("")
def update_profile(user_id: str, data: UpdateProfile, auth: Auth):
    token, user_supabase = auth
    response = (
        user_supabase.table("profile")
